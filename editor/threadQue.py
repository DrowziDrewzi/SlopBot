    ...
    def run(self):
        if self._target is not None:
            ...
            if self.wrapper is not None:
                ...
                if (
                    len(self.wrapper.que) > 0
                    and len(self.wrapper.threads) < self.wrapper.threadCount
                ):
                    self.wrapper.que.remove(q := self.wrapper.que[0])
                    tt = returnThreadWithQueUpdate(
                        *q.args, wrapper=self.wrapper, **q.kwargs
                    )
                    tt.start()
...
class threadQue:
    def __init__(
        ...
        self.que, self.threads, self.threadCount, self.useWrapper, self.timeout = (
            que,
            threads,
            threadCount,
            useWrapper,
    ...
    def runQuedThreads(self):
        ...
        for t in self.threads:
            if killThread := currentTime - t.startTime >= self.timeout:
                t.raise_exception(
                    f"Thread took longer than {self.timeout} second(s) to finish! Killing thread."
                )
