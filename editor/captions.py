        ...
        for i in range(-out, out + 1):
            for o in range(-out, out + 1):
                if (i == 0) or (o == 0):
                    continue
                canvas.text(
                    (int(x + i), int(y + o)), text, outColor, font=font, **kwargs
                )
    canvas.text(
        (int(x), int(y)), text, inColor, font=font, embedded_color=True, **kwargs
    )
    return
...
def normalcaption(width, height, cap=None, resourceDir="."):
    ...
    img = pad(
        crop := pad(
            cropWhite(img), int(width / 60), int(height / 60), color=(255, 255, 255)
        ),
        int((width - crop.size[0]) / 2),
        0,
        xOff=0,
        color=(255, 255, 255),
    )
...
def poster(width, height, cap=None, bottomcap=None, resourceDir="."):
    ...
    canvas.rectangle(
        [(1 + MP / 20, 1 + MP / 20), (width + MP / 20 + 2 * ol, bb + 1)],
        fill="black",
        width=ol,
        outline="white",
    )
    ...
    if bottomcap:
        ...
        canvas.text(
            (
                int((newSize[0] - smallW) / 2),
                int(nb := (bottom + 0.02 * MP if bottom else int(bb + MP / 9.5))),
            ),
            bottomcap,
            font=font,
            embedded_color=True,
...
def cap(width, height, cap=None, resourceDir="."):
    ...
    cropSize = crop.size
    img = pad(
        crop, int((width - cropSize[0]) / 2), int(0.1 * height), color=(255, 255, 255)
    )
    return fixSize(img)
