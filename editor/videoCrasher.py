...
def videoCrasher(
    videoName="monke.mp4", appendName="append.mp4", outputName="output.mp4"
):
    deleteOG = False
    if videoName == outputName:
        os.rename(
            deleteOG := videoName,
            videoName := (
                os.path.split(os.path.abspath(videoName))[0]
                + "/"
    ...
    try:
        subprocess.run(
            [
                ...
                "-pix_fmt",
                "yuv411p",
                newVidName := (tmpDir + "/" + os.path.split(videoName)[1] + "new.mp4"),
            ]
        )
