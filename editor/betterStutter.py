...
def reverseByChunks(l, s):
    return list(
        chain.from_iterable(
            [
                l[max(0, len(l) - i - s) : min(len(l), len(l) - i)]
...
def shuffleChunks(l, s):
    return list(
        chain.from_iterable(
            [l[i : i + s] for i in shuffle(list(range(ceil(len(l) / s))))]
        )
    )
...
def stutter(name, para, hasAudio):
    ...
    if para["shuffle"]:
        fancySort(para["shuffle"], filter_shuffle)
    if para["reverse"]:
        fancySort(para["reverse"], filter_reverse, intv=r(1, 4))
    if para["duplicate"]:
        fancySort(para["duplicate"], filter_duplicate, intv=r(1, 3))
    ...
    if hasAudio:
        ...
        for i in frames:
            ...
            if i.data == "shuffle":
                ...
                pattern = shuffleChunks(
                    list(range(len(part))), int(max(1, 50 - para["audioShuffle"] / 2))
                )
                for x in pattern:
                    newAudio += part[x]
                part = newAudio
                ...
                part = (
                    part[: para["audioDuplicate"]]
                    * ceil(len(part) / max(1, para["audioDuplicate"]))
                )[: len(part)]
            audioFrames += part
