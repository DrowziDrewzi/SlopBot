	...
	Autotune <link or search query> - Autotune a video (attached, replied to, or recent in channel) to another video."""
        ...
        if attach:
            if getExt(attach.filename) in exts:
                if (
                    len(spl := sub(" +", " ", message.content.strip()).split(" ", 1))
                    > 1
                ):
                    ...
                    result = await loop.run_in_executor(
                        ThreadPoolExecutor(), autotuneURL, fileName, spl[1]
                    )
                    if type(result) == str:
                        try:
                            await message.channel.send(
                                "Autotuning go ETH! 0x013d1361177ab72b0cf096bd34fa671efb3eeeee",
                            ...
                            print(e)
                            await message.channel.send(
                                "Sorry, something went wrong uploading your video. Maybe the file is too large?"
                            )
                    else:
                ...
                await message.channel.send(
                    "Invald file type, available formats: " + ", ".join(exts)
                )
        else:
